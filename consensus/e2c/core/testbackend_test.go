...
func (self *testSystemBackend) Validators(proposal e2c.Proposal) e2c.ValidatorSet {
	return self.peers
}
...
func (self *testSystemBackend) Send(message []byte, target common.Address) error {
	...
	self.sys.queuedMessage <- e2c.MessageEvent{
		Payload: message,
	}
	return nil
}

func (self *testSystemBackend) Broadcast(valSet e2c.ValidatorSet, message []byte) error {
	testLogger.Info("enqueuing a message...", "address", self.Address())
	self.sentMsgs = append(self.sentMsgs, message)
	self.sys.queuedMessage <- e2c.MessageEvent{
		Payload: message,
...
func (self *testSystemBackend) Gossip(valSet e2c.ValidatorSet, message []byte) error {
	testLogger.Warn("not sign any data")
	return nil
}

func (self *testSystemBackend) Commit(proposal e2c.Proposal, seals [][]byte) error {
	...
	go self.events.Post(e2c.FinalCommittedEvent{})
	return nil
}

func (self *testSystemBackend) Verify(proposal e2c.Proposal) (time.Duration, error) {
	return 0, nil
}
...
func (self *testSystemBackend) NewRequest(request e2c.Proposal) {
	go self.events.Post(e2c.RequestEvent{
		Proposal: request,
	})
}
...
func (self *testSystemBackend) LastProposal() (e2c.Proposal, common.Address) {
...
func (self *testSystemBackend) ParentValidators(proposal e2c.Proposal) e2c.ValidatorSet {
	return self.peers
}
...
type testSystem struct {
	...
	queuedMessage chan e2c.MessageEvent
	quit          chan struct{}
}
...
func newTestSystem(n uint64) *testSystem {
	...
	return &testSystem{
		...
		queuedMessage: make(chan e2c.MessageEvent),
...
func newTestValidatorSet(n int) e2c.ValidatorSet {
	return validator.NewSet(generateValidators(n), e2c.RoundRobin)
}
...
func NewTestSystemWithBackend(n, f uint64) *testSystem {
	...
	sys := newTestSystem(n)
	config := e2c.DefaultConfig

	for i := uint64(0); i < n; i++ {
		vset := validator.NewSet(addrs, e2c.RoundRobin)
		...
		core.current = newRoundState(&e2c.View{
			Round:    big.NewInt(0),
