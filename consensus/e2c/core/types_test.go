...
func testPreprepare(t *testing.T) {
	...
	var decodedPP *e2c.Preprepare
	err = decodedMsg.Decode(&decodedPP)
	if err != nil {
		t.Errorf("error mismatch: have %v, want nil", err)
	}

	// if block is encoded/decoded by rlp, we cannot to compare interface data type using reflect.DeepEqual. (like e2c.Proposal)
...
func testSubjectWithSignature(t *testing.T) {
	...
	decodedMsg = new(message)
	err = decodedMsg.FromPayload(msgPayload, func(data []byte, sig []byte) (common.Address, error) {
		return common.Address{}, e2c.ErrUnauthorizedAddress
	})
	if err != e2c.ErrUnauthorizedAddress {
		t.Errorf("error mismatch: have %v, want %v", err, e2c.ErrUnauthorizedAddress)
	}
}
