...
func newRoundState(view *e2c.View, validatorSet e2c.ValidatorSet, lockedHash common.Hash, preprepare *e2c.Preprepare, pendingRequest *e2c.Request, hasBadProposal func(hash common.Hash) bool) *roundState {
	return &roundState{
		round:          view.Round,
...
type roundState struct {
	round          *big.Int
	sequence       *big.Int
	Preprepare     *e2c.Preprepare
	Prepares       *messageSet
	Commits        *messageSet
	lockedHash     common.Hash
	pendingRequest *e2c.Request
...
func (s *roundState) Subject() *e2c.Subject {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *roundState) SetPreprepare(preprepare *e2c.Preprepare) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *roundState) Proposal() e2c.Proposal {
	s.mu.RLock()
	defer s.mu.RUnlock()
