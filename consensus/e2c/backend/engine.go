...
func (sb *backend) Signers(header *types.Header) ([]common.Address, error) {
	...
	proposalSeal := e2cCore.PrepareCommittedSeal(header.Hash())

	// 1. Get committed seals from current header
	for _, seal := range extra.CommittedSeal {
		// 2. Get the original address by seal and parent block hash
		addr, err := e2c.GetSignatureAddress(proposalSeal, seal)
		if err != nil {
...
func (sb *backend) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	fmt.Println("HERERERERERERERERRE\n\n\n\n\n\n\n\n\n\n\n\n")
...
func (sb *backend) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "e2c",
		Version:   "1.0",
		Service:   &API{chain: chain, e2c: sb},
		Public:    true,
	}}
}
...
func (sb *backend) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
	...
	for snap == nil {
		...
		if number == 0 {
			...
			e2cExtra, err := types.ExtractIstanbulExtra(genesis)
			if err != nil {
				return nil, err
			}
			snap = newSnapshot(sb.config.Epoch, 0, genesis.Hash(), validator.NewSet(e2cExtra.Validators, sb.config.ProposerPolicy))
...
func ecrecover(header *types.Header) (common.Address, error) {
	...
	e2cExtra, err := types.ExtractIstanbulExtra(header)
	if err != nil {
		return common.Address{}, err
	}
...
func writeSeal(h *types.Header, seal []byte) error {
	...
	e2cExtra, err := types.ExtractIstanbulExtra(h)
	if err != nil {
		return err
	}
...
func writeCommittedSeals(h *types.Header, committedSeals [][]byte) error {
	...
	e2cExtra.CommittedSeal = make([][]byte, len(committedSeals))
	copy(e2cExtra.CommittedSeal, committedSeals)
