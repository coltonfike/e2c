...
func appendValidators(genesis *core.Genesis, addrs []common.Address) {
	...
	if err != nil {
		panic("failed to encode e2c extra")
	}
	genesis.ExtraData = append(genesis.ExtraData, istPayload...)
}

func makeHeader(parent *types.Block, config *e2c.Config) *types.Header {
	header := &types.Header{
...
func TestSealStopChannel(t *testing.T) {
	...
	eventSub := engine.EventMux().Subscribe(e2c.RequestEvent{})
	eventLoop := func() {
		ev := <-eventSub.Chan()
		_, ok := ev.Data.(e2c.RequestEvent)
		if !ok {
...
func TestSealCommittedOtherHash(t *testing.T) {
	...
	go func() {
		ev := <-eventSub.Chan()
		if _, ok := ev.Data.(e2c.RequestEvent); !ok {
			t.Errorf("unexpected event comes: %v", reflect.TypeOf(ev.Data))
		}
