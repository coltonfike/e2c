...
type Snapshot struct {
	...
	ValSet e2c.ValidatorSet         // Set of authorized validators at this moment
}

// newSnapshot create a new snapshot with the specified startup parameters. This
// method does not initialize the set of recent validators, so only ever use if for
// the genesis block.
func newSnapshot(epoch uint64, number uint64, hash common.Hash, valSet e2c.ValidatorSet) *Snapshot {
	snap := &Snapshot{
		Epoch:  epoch,
		Number: number,
		Hash:   hash,
...
type snapshotJSON struct {
	...
	Policy     e2c.ProposerPolicy `json:"policy"`
}
