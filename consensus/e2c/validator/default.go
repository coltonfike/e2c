...
type defaultSet struct {
	...
	policy     e2c.ProposerPolicy

	proposer    e2c.Validator
	validatorMu sync.RWMutex
	selector    e2c.ProposalSelector
}

func newDefaultSet(addrs []common.Address, policy e2c.ProposerPolicy) *defaultSet {
	valSet := &defaultSet{}

	valSet.policy = policy
	// init validators
	valSet.validators = make([]e2c.Validator, len(addrs))
...
func (valSet *defaultSet) List() []e2c.Validator {
	valSet.validatorMu.RLock()
	defer valSet.validatorMu.RUnlock()
	return valSet.validators
}

func (valSet *defaultSet) GetByIndex(i uint64) e2c.Validator {
	valSet.validatorMu.RLock()
	defer valSet.validatorMu.RUnlock()
...
func (valSet *defaultSet) GetByAddress(addr common.Address) (int, e2c.Validator) {
...
func (valSet *defaultSet) GetProposer() e2c.Validator {
...
func calcSeed(valSet e2c.ValidatorSet, proposer common.Address, round uint64) uint64 {
	offset := 0
...
func roundRobinProposer(valSet e2c.ValidatorSet, proposer common.Address, round uint64) e2c.Validator {
	if valSet.Size() == 0 {
		return nil
	}
	seed := uint64(0)
...
func stickyProposer(valSet e2c.ValidatorSet, proposer common.Address, round uint64) e2c.Validator {
	if valSet.Size() == 0 {
		return nil
	}
	seed := uint64(0)
...
func (valSet *defaultSet) Copy() e2c.ValidatorSet {
	valSet.validatorMu.RLock()
	defer valSet.validatorMu.RUnlock()
...
func (valSet *defaultSet) Policy() e2c.ProposerPolicy { return valSet.policy }
